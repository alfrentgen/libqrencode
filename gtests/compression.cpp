...
void getPath(const coordinate_t start_coord, const path_t path_desc, std::vector<coordinate_t>& coords, uint8_t qr_size)
{
    ASSERT_TRUE(start_coord.x <= qr_size && start_coord.y <= qr_size);
    coords.clear();
    CyclicCounter<uint8_t> x_coord(0, qr_size, start_coord.x);
    CyclicCounter<uint8_t> y_coord(0, qr_size, start_coord.y);
...
void getRefLine(const QrCode& ref_qr, const ref_line_descriptor_t& ref_line_desc, std::bitset<QR_SIZE>& ref_line)
{
...
template<uint8_t QR_SIZE>
void compress(const QrCode& ref, const QrCode& comp, std::vector<line_compression_t>& line_compressions)
{
    ...
    for(uint8_t comp_y {0}; comp_y < QR_SIZE; ++comp_y) {
        ...
        [&](){
            for(uint8_t ref_y {0}; ref_y < QR_SIZE && !stop_search; ++ref_y) {
                for(uint8_t ref_x {0}; ref_x < QR_SIZE && !stop_search; ++ref_x) {
                    for(int8_t y_shift {MIN_SHIFT}; y_shift <= MAX_SHIFT; ++y_shift) {
                        for(int8_t x_shift {MIN_SHIFT}; x_shift <= MAX_SHIFT; ++x_shift) {
                            ...
                            for(uint8_t step {0}; step <= MAX_STEP; ++step) {
                                ...
                                getRefLine<QR_SIZE>(ref, ref_line_desc, ref_line);
